...
async function checkReminders() {
  ...
  for (const pending of pendings) {
    try {
      ...
      await channel.send({
        content: `🔔 ${mentions}`,
        embeds: [new EmbedBuilder().setTitle('تذكير: لم يتم اتخاذ إجراء').setDescription(`العضو <@${pending.userId}> فقد البوست منذ 3 أيام`).setColor(0xFFA500)],
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
  if (oldMember.premiumSince &&!newMember.premiumSince) {
    ...
    const embed = new EmbedBuilder().setTitle('⚠ داعم ألغى البوست').setDescription(`**العضو:** ${newMember}\n**الرول:** ${role || 'محذوف'}\n**الأعضاء:** ${JSON.parse(data.members).length}/3`).setColor(0xFF0000).setThumbnail(newMember.user.displayAvatarURL()).setTimestamp();
    ...
    db.prepare('INSERT OR REPLACE INTO pending_boost_loss VALUES (?,?,?,?,?,?,?,?)').run(newMember.id, data.roleId, newMember.guild.id, logChannel.id, msg.id, Date.now(), 0, null);
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'setup') {
    ...
    const menu = new StringSelectMenuBuilder().setCustomId('admin_setup').setPlaceholder('يرجى تحديد نوع التعديل..').addOptions([
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'roles') {
    ...
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('open_user_panel').setEmoji('⚙').setStyle(ButtonStyle.Secondary));
    await interaction.channel.send({ embeds: [embed], components: [row] });
    return interaction.editReply({ content: '✅ تم إرسال اللوحة' });
  }
  ...
  if (interaction.isButton() && interaction.customId === 'open_user_panel') {
    ...
    const row1 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('create_role').setLabel('انشاء رول').setStyle(ButtonStyle.Secondary).setEmoji('➕'), new ButtonBuilder().setCustomId('add_role_member').setLabel('اضافة عضو').setStyle(ButtonStyle.Secondary).setEmoji('📝'));
    const row2 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('remove_role_member').setLabel('ازالة عضو').setStyle(ButtonStyle.Secondary).setEmoji('➖'), new ButtonBuilder().setCustomId('delete_role').setLabel('حذف رول').setStyle(ButtonStyle.Secondary).setEmoji('🗑'));
    const row3 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('role_info').setLabel('معلومات رول').setStyle(ButtonStyle.Secondary).setEmoji('⚙'));
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === 'admin_setup') {
    ...
    if (value === 'list_roles') {
      ...
      return interaction.reply({ ephemeral:true, embeds: [new EmbedBuilder().setTitle('قائمة الرولات').setDescription(text).setColor(0x2B2D31)] });
    }
    if (value === 'toggle_manager') {
      const modal = new ModalBuilder().setCustomId('toggle_manager').setTitle('اضافة/ازالة مسؤول');
      modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('id').setLabel('منشن أو ID العضو').setStyle(TextInputStyle.Short).setRequired(true)));
      return interaction.showModal(modal);
    }
    if (value === 'list_managers') {
      const managers = JSON.parse(getSetting('managers') || '[]');
      const text = managers.map(id => `<@${id}>`).join('\n') || 'لا يوجد مسؤولين';
      return interaction.reply({ ephemeral:true, embeds: [new EmbedBuilder().setTitle('قائمة المسؤولين').setDescription(text).setColor(0x2B2D31)] });
    }
    if (value === 'set_log') { setSetting('logChannel', interaction.channelId); return interaction.reply({ content: '✅ تم تحديد هذا الروم كـ لوق للرولات', ephemeral:true }); }
    if (value === 'set_new_roles') { setSetting('newRolesChannel', interaction.channelId); return interaction.reply({ content: '✅ تم تحديد هذا الروم لإشعارات الرولات الجديدة', ephemeral:true }); }
    if (value === 'send_user_panel') {
      const embed = new EmbedBuilder().setDescription('للتحكم بالرول الخاص بك الضغط على الزر.').setColor(0x2B2D31);
      const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('open_user_panel').setEmoji('⚙').setStyle(ButtonStyle.Secondary));
      await interaction.channel.send({ embeds: [embed], components: [row] });
      return interaction.reply({ content: '✅ تم إرسال لوحة الداعمين', ephemeral:true });
    }
    if (value === 'send_admin_panel') { return interaction.reply({ content: '✅ استخدم /setup', ephemeral:true }); }
    if (value === 'toggle_allowed') {
      const modal = new ModalBuilder().setCustomId('toggle_allowed').setTitle('شات مصرح');
      modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('id').setLabel('ID الروم').setStyle(TextInputStyle.Short).setRequired(true)));
      return interaction.showModal(modal);
    }
    ...
    if (value === 'list_channels') {
      ...
      return interaction.reply({ ephemeral:true, embeds: [new EmbedBuilder().setTitle('قائمة الشاتات').setDescription(text).setColor(0x2B2D31)] });
    }
  }
  ...
  if (interaction.isButton() && interaction.customId === 'create_role') {
    ...
    modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('name').setLabel('اسم الرول').setStyle(TextInputStyle.Short).setMaxLength(32).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('color').setLabel('اللون (مثال: #FF0000 أو #FF0000,#00FF00)').setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('icon').setLabel('الشعار (رابط أو إيموجي)').setStyle(TextInputStyle.Short).setRequired(false)));
    return interaction.showModal(modal);
  }
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'role_create') {
    ...
    const color1 = colors[0]; const color2 = colors[1] || null;
    ...
    try {
      ...
      const newChannel = getSetting('newRolesChannel'); if (newChannel) { const ch = await interaction.guild.channels.fetch(newChannel).catch(() => null); if (ch) ch.send(`✨ رول جديد: ${role} بواسطة ${interaction.user}`); }
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'add_member') {
    ...
    members.push(id); await member.roles.add(role);
