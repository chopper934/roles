...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
  if (oldMember.premiumSince &&!newMember.premiumSince) {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`delete_role_${data.userId}`).setLabel('حذف الرول').setStyle(ButtonStyle.Danger).setEmoji('🗑️'),
    ...
    await logAction(newMember.guild, `⚠️ ${newMember.user.tag} فقد البوست`);
  }
});
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'setup') {
    ...
    const menu = new StringSelectMenuBuilder()
     ...
     .addOptions([
        { label: 'قائمة الرولات', value: 'list_roles', description: 'لعرض قائمة الرولات المضافه بالبوت.', emoji: '🗄️' },
        ...
        { label: 'رسالة التحكم بالرول', value: 'send_user_panel', description: 'لارسال رسالة التحكم بالرول الخاص من الزر.', emoji: '⚙️' },
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'roles') {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('open_user_panel').setEmoji('⚙️').setStyle(ButtonStyle.Secondary)
    );
  ...
  if (interaction.isButton() && interaction.customId === 'open_user_panel') {
    ...
    const row2 = new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder().setCustomId('delete_role').setLabel('حذف رول').setStyle(ButtonStyle.Secondary).setEmoji('🗑️')
    );

    const row3 = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('role_info').setLabel('معلومات رول').setStyle(ButtonStyle.Secondary).setEmoji('⚙️')
    );
  ...
  if (interaction.isButton() && interaction.customId === 'create_role') {
    ...
    if (allowed.length > 0 &&!allowed.includes(interaction.channelId)) {
      return interaction.reply({ content: '❌ هذا الروم غير مصرح', ephemeral: true });
    }
    if (blocked.includes(interaction.channelId)) {
      return interaction.reply({ content: '❌ هذا الروم محظور', ephemeral: true });
    }
    if (!isBooster(member)) {
      return interaction.reply({ content: '❌ يجب أن تكون بوستر', ephemeral: true });
    }
    if (db.prepare('SELECT * FROM roles WHERE userId =?').get(interaction.user.id)) {
      return interaction.reply({ content: '❌ لديك رول بالفعل', ephemeral: true });
    }
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'role_create') {
    ...
    try {
      const role = await interaction.guild.roles.create({
        ...
        color: color1,
        icon,
        reason: `رول خاص لـ ${interaction.user.tag}`
      });

      await interaction.member.roles.add(role);

      db.prepare('INSERT INTO roles VALUES (?,?,?,?,?,?,?,?,?)').run(
        interaction.user.id, role.id, name, color1, color2, icon,
        JSON.stringify([interaction.user.id]), Date.now(), interaction.user.id
      );

      const newChannel = getSetting('newRolesChannel');
      if (newChannel) {
        const ch = await interaction.guild.channels.fetch(newChannel).catch(() => null);
        if (ch) ch.send(`✨ رول جديد: ${role} بواسطة ${interaction.user}`);
      }
  ...
  if (interaction.isButton() && interaction.customId === 'add_role_member') {
    ...
    if (members.length >= 3) {
      return interaction.reply({ content: '❌ وصلت للحد الأقصى (3 أعضاء)', ephemeral: true });
    }

    const modal = new ModalBuilder().setCustomId('add_member').setTitle('اضافة عضو');
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('id').setLabel('منشن العضو أو ID').setStyle(TextInputStyle.Short).setRequired(true)
    ));
  ...
  if (interaction.isButton() && interaction.customId === 'remove_role_member') {
    ...
    if (members.length === 0) {
      return interaction.reply({ content: '❌ لا يوجد أعضاء لإزالتهم', ephemeral: true });
    }

    const options = await Promise.all(members.map(async id => {
      const member = await interaction.guild.members.fetch(id).catch(() => null);
      return { label: member? member.user.tag : id, value: id };
    }));

    const menu = new StringSelectMenuBuilder()
     .setCustomId('remove_member_select')
     .setPlaceholder('اختر عضو لإزالته')
     .addOptions(options);
  ...
  if (interaction.isButton() && interaction.customId === 'delete_role') {
    ...
    await logAction(interaction.guild, `🗑️ ${interaction.user} حذف روله`);
  ...
  if (interaction.isButton() && interaction.customId === 'role_info') {
    ...
    const embed = new EmbedBuilder()
     .setTitle(`معلومات رول: ${role.name}`)
     .setColor(data.color)
     .addFields(
        { name: 'الاسم', value: role.name, inline: true },
        { name: 'اللون', value: data.color2? `${data.color} → ${data.color2}` : data.color, inline: true },
        { name: 'الأعضاء', value: `${members.length}/3\n${membersList}`, inline: false },
        { name: 'تاريخ الإنشاء', value: `<t:${Math.floor(data.createdAt / 1000)}:F>`, inline: false }
      );
  ...
  if (interaction.isButton() && interaction.customId.startsWith('delete_role_')) {
    if (!isManager(interaction.user.id) &&!interaction.memberPermissions.has(PermissionFlagsBits.Administrator)) {
      return interaction.reply({ content: '❌', ephemeral: true });
    }
    ...
    if (data) {
      const role = await interaction.guild.roles.fetch(data.roleId).catch(() => null);
      if (role) await role.delete('فقدان بوست');
    ...
    await logAction(interaction.guild, `🗑️ **حذف رول**\nالعضو: <@${userId}>\nبواسطة: ${interaction.user}`);
  }
